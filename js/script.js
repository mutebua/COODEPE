...
// lightbox plugin
$(document).ready(function() {
    var url = window.location.pathname; // Obtenez le chemin de l'URL actuelle
    var filename = url.substring(url.lastIndexOf('/')+1); // Extraire le nom du fichier

    $('.navbar-nav li a').each(function() {
        var href = $(this).attr('href'); // Obtenir le lien href
        if (href === filename) { // Si le lien correspond au nom du fichier
            $(this).parent().addClass('active'); // Ajouter la classe active
        }
    });
});
